...
class EmployeeDto
{
	 ...
	 * @return mixed
	 */
	public function employeeAddXml()
	{
		$root = new \SimpleXMLElement('<employee></employee>');
		$root->addChild('id', $this->getId());
		$root->addChild('code', $this->getCode());
		$root->addChild('name', htmlspecialchars($this->getName()));
		$root->addChild('deleted', $this->isDeleted() ? "true" : "false");
		$root->addChild('supplier', $this->isSupplier() ? "true" : "false");
		$root->addChild('employee', $this->isEmployee() ? "true" : "false");
		$root->addChild('login', htmlspecialchars($this->getLogin()));
		$root->addChild('client', $this->isClient() ? "true" : "false");
		if ($this->getEmail())
		{
			$root->addChild('email', htmlspecialchars($this->getEmail()));
		}

		if ($this->getMainRoleCode())
		{
			$root->addChild('mainRoleCode', htmlspecialchars($this->getMainRoleCode()));
		}
		if ($this->getPreferredDepartmentCode())
		{
			$root->addChild('preferredDepartmentCode', $this->getPreferredDepartmentCode());
		}
		if ($this->getPhone())
		{
			$root->addChild('phone', htmlspecialchars($this->getPhone()));
		}
		if ($this->getCellPhone())
		{
			$root->addChild('cellPhone', htmlspecialchars($this->getCellPhone()));
		}
		if ($this->getFirstName())
		{
			$root->addChild('firstName', htmlspecialchars($this->getFirstName()));
		}
		if ($this->getMiddleName())
		{
			$root->addChild('middleName', htmlspecialchars($this->getMiddleName()));
		}
		if ($this->getLastName())
		{
			$root->addChild('lastName', htmlspecialchars($this->getLastName()));
		}
		if ($this->getCardNumber())
		{
			$root->addChild('cardNumber', htmlspecialchars($this->getCardNumber()));
		}
		if ($this->getTaxpayerIdNumber())
		{
			$root->addChild('taxpayerIdNumber', htmlspecialchars($this->getTaxpayerIdNumber()));
		}
		if ($this->getSnils())
		{
			$root->addChild('snils', htmlspecialchars($this->getSnils()));
		}
		if ($this->getAddress())
		{
			$root->addChild('address', htmlspecialchars($this->getAddress()));
		}

		if ($this->getRoleCodes())
		{
			foreach ($this->getRoleCodes() as $roleCode)
			{
				$root->addChild('roleCodes', $roleCode);
			}
		}
		if ($this->getDepartmentCodes())
		{
			foreach ($this->getDepartmentCodes() as $code)
			{
				$root->addChild('departmentCodes', $code);
			}
		}
		if ($this->getResponsibilityDepartmentCodes())
		{
			foreach ($this->getResponsibilityDepartmentCodes() as $departmentCode)
			{
				$root->addChild('responsibilityDepartmentCodes', $departmentCode);
			}
		}
		if (!is_null($this->getNote()))
		{
			$root->addChild('note', htmlspecialchars($this->getNote()));
		}
		if ($this->getGln())
		{
			$root->addChild('gln', $this->getGln());
		}
		if ($this->getActivationDate())
		{
			$root->addChild('activationDate', $this->getActivationDate()->format(self::DATE_FORMAT));
		}

		if ($this->getDeactivationDate())
		{
			$root->addChild('deactivationDate', $this->getDeactivationDate()->format(self::DATE_FORMAT));
		}
		if ($this->getBirthday())
		{
			$root->addChild('birthday', $this->getBirthday()->format(self::DATE_FORMAT));
		}
		if ($this->getHireDate())
		{
			$root->addChild('hireDate', $this->getHireDate()->format(self::DATE_FORMAT));
		}
		if ($this->getHireDocumentNumber())
		{
			$root->addChild('hireDocumentNumber', htmlspecialchars($this->getHireDocumentNumber()));
		}
		if ($this->getPinCode())
		{
			$root->addChild('pinCode', $this->getPinCode());
		}
		if ($this->getPassword())
		{
			$root->addChild('password', $this->getPassword());
		}
		return $root->asXML();
	}
